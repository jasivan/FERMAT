...
parser.add_argument('--route',
                    default='../../../fastdata/acp21jas/DROP/',
...
def preprocess_function(examples):
    ...
    for question, answer in zip(examples['question'], examples['answer']):
      inputs = char_level_representation_encoding(question, tokenizer=tokenizer, max_length=max_input_length)
      target = char_level_representation_encoding(answer, tokenizer=tokenizer, max_length=max_input_length)
      inputs_ids.append(inputs['input_ids'])
      attention_mask.append(inputs['attention_mask'])
      questions.append(question)
      answers.append(answer)
...
import csv
with open(args.output+'/drop_results.txt', 'w') as f:
  f.write(str(output[2]))

header = ['preds', 'targets']
data = [ [str(pred), str(label)] for pred, label in zip(preds, labels) ]

with open(args.output+f'/{model_name}_preds_labels.csv', 'w', newline='') as f:
    writer = csv.writer(f)
