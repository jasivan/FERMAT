...
# happy_gen = HappyGeneration("GPT-new", "EleutherAI/gpt-neo-125M")
# result = happy_gen.generate_text("Compute 45+52. ")
# print(result)
# print(result.text)

# from happytransformer import GENSettings
# args = GENSettings(no_repeat_ngram_size=10)
...
# args = GENTrainArgs(num_train_epochs=20)
# happy_gen.train("data/mini/gpt/train.txt", args=args)
...
args = QATrainArgs(num_train_epochs=20)
...
#       pred = pred.replace('<pad>', '').replace('</s>', '').replace('<s>', '').replace(' ', '')
#       label = label.replace('<pad>', '').replace('</s>', '').replace('<s>', '').replace(' ', '')
...
#     pred = raw_pred.replace('<pad>', '').replace('</s>', '').replace('<s>', '').replace(' ', '')
#     raw_label = tokenizer.decode(np.where(output[1][i] == -100, 0, output[1][i]))
#     label = raw_label.replace('<pad>', '').replace('</s>', '').replace('<s>', '').replace(' ', '')
...
# print('test_accuracy=', acc/len(output[0]))
