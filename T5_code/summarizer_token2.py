...
tokenizer = AutoTokenizer.from_pretrained(model_checkpoint, model_max_length=max_input_length)#, additional_special_tokens=['[NUM]', '[^NUM]'])
# num_added_toks = tokenizer.add_tokens(['[NUM]', '[^NUM]'], special_tokens=True)



def spaced_out(text):
  text = re.sub(r'(\d)(\.|\?) ', '\g<1> \g<2> ', text)
  text = re.sub(r'(^|[^0-9 \.,])(\d)', '\g<1> \g<2>',text)
  nums = re.findall(r' (\d+\.\d+|\d+,\d+|\d+ \d+|\d+) ', text)
  ...
  for num in nums:
    ...
    for i in num:
      spaced_num += i+' '
    spaced_num = ' [NUM] ' + spaced_num.strip(' ') + ' [^NUM] '
    text = text.replace(' '+num+' ', spaced_num, 1)
  return text

def preprocess_function(examples):
    inputs = [prefix + spaced_out(doc) for doc in examples["question"]]
    model_inputs = tokenizer(inputs, max_length=max_input_length, truncation=True)

    # Setup the tokenizer for targets
    outputs = [spaced_out(doc) for doc in examples["answer"]]
